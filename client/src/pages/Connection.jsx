...
const connection = () => {
  ...
  return (
      ...
      {dataArray.map((tab)=>(
        <button onClick={()=>setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 cursor-pointer text-sm rounded-md transition-colors ${currentTab===tab.label ? "bg-white font-medium text-black" :"text-gray-500 hover:text-black"}`}>
      ...
      <div className="flex flex-wrap gap-6 mt-6">
       ...
       <div key={user._id} className="w-full max-w-80 flex gap-5 p-6 bg-white shadow rounded-md">
